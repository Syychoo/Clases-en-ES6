  ...
  while (!exit) {
    ...
    switch (option) {
      case 1:
        ...
        let n_taxes = parseInt(
          prompt("Indique el numero de impuestos que va a ingresar: ")
        );

        const client = new Client(name);

        for (let i = 0; i < n_taxes; i++) {
          let annualGrossAmount = parseFloat(
            prompt(`Ingrese el monto bruto anual del impuesto ${i}: `)
          );
          let deductions = parseFloat(
            prompt(`Ingrese las deducciones del impuesto ${i}: `)
          );
        ...
        clients.push(client);
        alert(
          `Se ha creado el cliente ${client.name} con exito!\nLos impuestos se mostraran por consola...`
        );
      ...
      case 2:
        ...
        alert(
          `El impuesto total a pagar del cliente ${clientName} es: $${clientFound.calculateTax()}`
        );
